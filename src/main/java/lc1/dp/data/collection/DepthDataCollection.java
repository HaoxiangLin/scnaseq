...
public class DepthDataCollection extends LikelihoodDataCollection{
  ...
  @Override
    public final Boolean process(String indiv, String[] header,  String[] geno, int i, int ploidy, double[] missing){
	  ...
	  boolean naR = false;
       ...
       IlluminaRDistribution dist =  new IlluminaRDistribution(this.index);
...
//    	    return state.emissions[i].probeOnly();
    ...
    public  DepthDataCollection (File f, short index, int no_copies, 
    		int[][] mid,  File bf,Collection<String> snpidrest) throws Exception{
        super(f, index, no_copies, mid, bf,snpidrest);
        
    }
 
    public DepthDataCollection(DepthDataCollection collection) {
      super(collection);
      this.dc = collection.dc;
     
    }
    public DepthDataCollection(DataCollection obj) {
        super(obj);
   //     this.make();
       // this.stSp = obj.stSp;
...
public final void makeDistributions(int index) {
    ...
    public DataC clone(){
        return new DepthDataCollection(this);
    }
