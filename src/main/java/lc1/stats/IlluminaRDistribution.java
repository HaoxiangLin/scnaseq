...
public class IlluminaRDistribution extends PseudoDistribution{
 protected Double r=null;
    //boolean isnan =false;
    public IlluminaRDistribution(short data_index){
    ...
    public boolean equals(PseudoDistribution dist){
    	if(dist.getClass().equals(this.getClass())){
    		if(super.equals(dist) && this.r.equals(((IlluminaRDistribution)dist).r())) return true;
    	}
    	return false;
    }
  //}
    ...
    public IlluminaRDistribution(IlluminaRDistribution ill) {
       this.r = ill.r;
 ...
 /*   public void addCount(IlluminaProbB[] probB, Integer index, double val,int pos){
   ...
   /* private double score(int j, IlluminaProbB probB, IlluminaProbR probR, EmissionStateSpace emStsp, int pos) {
    ...
    public PseudoDistribution clone() {
       return new IlluminaRDistribution(this);
    }
    ...
    public void setR(Double r2){
        if(r2==null || Double.isNaN(r2) ){
        	...
        	if(r!=null && Constants.cumulativeR(this.data_index)>1){
        	   this.r = r2+ this.r;	
        	}
        	//if(r2>0.95){
...
public void compareTo(PseudoDistribution pseudoDistribution) {
    if(pseudoDistribution instanceof IlluminaRDistribution){
    
     if(Math.abs(this.r - ((IlluminaRDistribution)pseudoDistribution).r)>0.75){
         throw new RuntimeException("r values are very different ");
     }  
    }
}
public void dropROutlier(double[] ds) {
if(r==null || this.r < ds[0] || this.r>ds[1]) r = null;
...
public double scoreBR(EmissionStateSpace emstsp, int j,  int i, int mixComponent) {
	...
	int noCop = emstsp.getCN(j);
		//return 
	return Math.pow(
    			DistributionCollection.dc.getDistributionRB(this.data_index, noCop,noB, i).probability(this.r, 0, mixComponent)
    			,Constants.exponentB(data_index))
    			;
		/*return scoreR(Constants.backgroundCount(), EmissionStateSpace.emStSp.noCopies(),i);*/
	}
...
public void divide(double cumR) {
	this.r = this.r/cumR;
