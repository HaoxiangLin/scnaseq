...
public class GammaDistribution
{
	 ...
	 * @return cdf value
	 */
	public static double cdfLog(double x, double shape, double scale)
	{
		double res =  pal.math.GammaFunction.incompleteGammaP(shape, x/scale);
		if(res==1.0) return GammaFunction.incompleteGammaPLog(shape, x/scale);
		else return Math.log10(1-res);
	}
	
	public static double cdf(double x, double shape, double scale)
	{
		return  pal.math.GammaFunction.incompleteGammaP(shape, x/scale);
	...
	private static double pointChi2(double prob, double v)
	{
		...
		do
		{
			q = ch;
			p1 = 0.5*ch;
			if ((t = pal.math.GammaFunction.incompleteGammaP(xx, p1, g)) < 0)
			{
